...
TOGGLE_BITS = 16            # number of bits to fuzz when searching fingerprint
                            # matches. Values: 0 - 22. (may increase robustness
                            # but increase false positive rate and system load)
...
def stft(data, win_len=100, win_overlap=10):
    ...
    for i in range(num_frames):
        ...
        stft_out[i,:] = np.square(np.abs(fft_frame[:win_len_frames//2])) \
                / win_len_frames
...
def compute_mel(data, nbins=18):
    ...
    filter_banks = \
            np.where(filter_banks <= 0, np.finfo(float).eps, filter_banks)
...
def find_peaks(data, query=True, ID=None):
    ...
    for t in range(9, data.shape[0]-9):
        for b in range(1, data.shape[1]-1): # bands 1-16 (inclusive)
            ...
            # region 3
            r_3a = (data[t-2][b+1] + data[t-1][b+1] \
                    + data[t][b+1] + data[t-2][b] + data[t-1][b]) / 5
            r_3b = (data[t][b+1] + data[t+1][b+1] \
                    + data[t+2][b+1] + data[t+1][b] + data[t+2][b]) / 5
            r_3c = (data[t+1][b] + data[t+2][b] \
                    + data[t][b-1] + data[t+1][b-1] + data[t+2][b-1]) / 5
            r_3d = (data[t-2][b] + data[t-1][b] \
                    + data[t-2][b-1] + data[t-1][b-1] + data[t][b-1]) / 5

            # region 4
            if b == data.shape[1] - 2:
                r_4a = (data[t-9][b+1] + data[t-8][b+1] \
                        + data[t-7][b+1] + data[t-6][b+1]) / 4
                r_4b = (data[t-5][b+1] + data[t-4][b+1] \
                        + data[t-3][b+1] + data[t-2][b+1]) / 4
                r_4e = (data[t+5][b+1] + data[t+4][b+1] \
                        + data[t+3][b+1] + data[t+2][b+1]) / 4
                r_4f = (data[t+9][b+1] + data[t+8][b+1] \
                        + data[t+7][b+1] + data[t+6][b+1]) / 4
            else:
                r_4a = (data[t-9][b+2] + data[t-8][b+2] + data[t-7][b+2] 
                        + data[t-6][b+2] + data[t-9][b+1] + data[t-8][b+1] \
                        + data[t-7][b+1] + data[t-6][b+1]) / 8
                r_4b = (data[t-5][b+2] + data[t-4][b+2] + data[t-3][b+2] \
                        + data[t-2][b+2] + data[t-5][b+1] + data[t-4][b+1] \
                        + data[t-3][b+1] + data[t-2][b+1]) / 8
                r_4e = (data[t+5][b+2] + data[t+4][b+2] + data[t+3][b+2] \
                        + data[t+2][b+2] + data[t+5][b+1] + data[t+4][b+1] \
                        + data[t+3][b+1] + data[t+2][b+1]) / 8
                r_4f = (data[t+9][b+2] + data[t+8][b+2] + data[t+7][b+2] \
                        + data[t+6][b+2] + data[t+9][b+1] + data[t+8][b+1] \
                        + data[t+7][b+1] + data[t+6][b+1]) / 8
            if b == 1:
                r_4c = (data[t-9][b-1] + data[t-8][b-1] \
                        + data[t-7][b-1] + data[t-6][b-1]) / 4
                r_4d = (data[t-5][b-1] + data[t-4][b-1] \
                        + data[t-3][b-1] + data[t-2][b-1]) / 4
                r_4g = (data[t+5][b-1] + data[t+4][b-1] \
                        + data[t+3][b-1] + data[t+2][b-1]) / 4
                r_4h = (data[t+9][b-1] + data[t+8][b-1] \
                        + data[t+7][b-1] + data[t+6][b-1]) / 4
            else:
                r_4c = (data[t-9][b-2] + data[t-8][b-2] + data[t-7][b-2] \
                        + data[t-6][b-2] + data[t-9][b-1] + data[t-8][b-1] \
                        + data[t-7][b-1] + data[t-6][b-1]) / 8
                r_4d = (data[t-5][b-2] + data[t-4][b-2] + data[t-3][b-2] \
                        + data[t-2][b-2] + data[t-5][b-1] + data[t-4][b-1] \
                        + data[t-3][b-1] + data[t-2][b-1]) / 8
                r_4g = (data[t+5][b-2] + data[t+4][b-2] + data[t-3][b-2] \
                        + data[t+2][b-2] + data[t+5][b-1] + data[t+4][b-1] \
                        + data[t+3][b-1] + data[t+2][b-1]) / 8
                r_4h = (data[t+9][b-2] + data[t+8][b-2] + data[t+7][b-2] \
                        + data[t+6][b-2] + data[t+9][b-1] + data[t+8][b-1] \
                        + data[t+7][b-1] + data[t+6][b-1]) / 8
...
def audio_callback(indata, frames, callback_time, status):
    ...
    for m in matches:
        ...
        for i in range(len(matches[m][0])):
            ...
            if (elapsed_sec < THRESHOLD_RATE_MIN_TIME and \
                    hist[dt] >= THRESHOLD_RATE_MIN) \
                    or hist[dt] >= THRESHOLD:
                temp_found = True
